...
CMD_F1="$LABEL 'IB Block duration of all input nodes in $JOB_NAME [$INPUT_COUNT INs, $COMPUTE_COUNT CNs, $INPUT_BW GB/s] [$DATE]'; plot "
...
while [  $COUNTER -lt $INPUT_FILES ]; do
	...
	CMD_F1="$CMD_F1'$FILE_NAME' using 1:(\$3 == 0?1/0:\$3) with points title 'duration $COUNTER' "
