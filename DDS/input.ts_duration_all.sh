...
LABEL="set xlabel 'Timeslice'; set ylabel 'Duration in ms'; set ytics 0,10; set title"
...
while [  $COUNTER -lt ${array[1]} ]; do
	...
	if [ ${#array[@]} -eq "2" ]; then
		CMD_F1="$CMD_F1'$FILE_NAME' using ((\$1-floor(\$1/100)*100) == 0?\$1:1/0):(\$3/1000.0) with points title 'I$COUNTER' "
