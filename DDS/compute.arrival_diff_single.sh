...
FILE_NAME="${array[0]}/${array[1]}.compute.arrival_diff.out"
...
LABEL=" set xlabel 'Timeslice no.' font 'Helvetica,35' offset 0,-5,0; 
	...
	#set lmargin 18;
	#set rmargin 7;"
CMD="$LABEL set title 'Diff between first and last contribution arrival of each timeslice of Compute Node ${array[0]} in $JOB_NAME [$INPUT_COUNT INs, $COMPUTE_COUNT CNs, $INPUT_BW GB/s] [$DATE]'; plot "
